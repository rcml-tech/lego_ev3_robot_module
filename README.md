# Модуль RCML для Lego EV3
-------------------------------
Данный модуль позволяет программировать **Lego** роботов на RCML, работающих под управлением **Lego блока EV3**. Программа на RCML исполняется на ПК, связь с EV3 осуществляется через **Bluetooth**.<br>
EV3 предварительно должен быть сопряжен с компьютером по Bluetooth.<br>
Модуль имеет конфигурационный файл, в котором указывается COM-порт, осуществляющий коммуникацию с EV3. Номер COM-порта можно посмотреть в свойствах Bluetooth устройства на ПК.<br>
...
*Например модуль EV3 подключен к порту COM3 и мы хотим разрешить динамичеси подключать роботов, тогда config.ini будет иметь следующее содержимое:*
```ini
    [connections]
    connection = COM3
    [options]
...
Для данного модуля доступно подключение датчиков 8-ми типов.
